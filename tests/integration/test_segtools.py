...
@pytest.mark.workflow("test_segtools")
def test_segtools_length_distribution_pdfs_match(
    ...
    expected = test_data_dir / Path("test_segtools_length_distribution.pdf")
...
def test_segtools_segment_sizes_pdfs_match(workflow_dir, test_data_dir, pdfs_match):
    ...
    expected = test_data_dir / Path("test_segtools_segment_sizes.pdf")
    assert pdfs_match(result, expected)
...
def test_segtools_gmtk_parameters_pdfs_match(workflow_dir, test_data_dir, pdfs_match):
    ...
    expected = test_data_dir / Path("test_segtools_gmtk_parameters.pdf")
    assert pdfs_match(result, expected)


@pytest.mark.workflow("test_segtools")
def test_segtools_feature_aggregation_splicing_pdfs_match(
    workflow_dir, test_data_dir, pdfs_match
):
    result = workflow_dir / Path(
        "test-output/glob-9a503dc39dbe819d5ebf7343f90bb109/feature_aggregation.splicing.pdf"
    )
    expected = test_data_dir / Path("test_segtools_feature_aggregation.splicing.pdf")
    assert pdfs_match(result, expected)


@pytest.mark.workflow("test_segtools")
def test_segtools_feature_aggregation_translation_pdfs_match(
    workflow_dir, test_data_dir, pdfs_match
):
    result = workflow_dir / Path(
        "test-output/glob-9a503dc39dbe819d5ebf7343f90bb109/feature_aggregation.translation.pdf"
    )
    expected = test_data_dir / Path("test_segtools_feature_aggregation.translation.pdf")
    assert pdfs_match(result, expected)
